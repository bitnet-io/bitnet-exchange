...
class TradeTableColumnSupplier {
    ...
    private final Predicate<OfferInfo> isFiatOffer = (o) -> o.getBaseCurrencyCode().equals("BIT");
    ...
    final Supplier<Column<Long>> bisqTradeDetailFeeColumn = () -> {
        if (isTradeDetailTblBuilder.get()) {
            ...
            String headerCurrencyCode = isTaker.test(t)
                    ? t.getIsCurrencyForTakerFeeBtc() ? "BIT" : "BSQ"
                    : t.getOffer().getIsCurrencyForMakerFeeBtc() ? "BIT" : "BSQ";
