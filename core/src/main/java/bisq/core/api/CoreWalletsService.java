...
class CoreWalletsService {
    ...
    void sendBtc(String address,
        ...
        try {
            ...
            log.info("Sending {} BIT to {} with tx fee of {} radiowaves (fee rate {} radiowaves/byte).",
                    amount,
                    address,
                    fee.value,
                    txFeePerVbyte.value);
            ...
            log.error(ex.toString());
            throw new IllegalStateException("cannot send bit from any addresses in wallet", ex);
        } catch (InsufficientFundsException | InsufficientMoneyException ex) {
            log.error(ex.toString());
            throw new NotAvailableException("cannot send bit due to insufficient funds", ex);
        }
    }
    ...
    private void verifyWalletCurrencyCodeIsValid(String currencyCode) {
        ...
        if (!currencyCode.equalsIgnoreCase("BSQ")
                && !currencyCode.equalsIgnoreCase("BIT"))
