...
public class Config {
    ...
    public static final int DEFAULT_NUM_CONNECTIONS_FOR_BTC_PROVIDED = 10; // down from BitnetJ default of 12
    public static final int DEFAULT_NUM_CONNECTIONS_FOR_BTC_PUBLIC = 10;
    public static final boolean DEFAULT_FULL_DAO_NODE = false;
    static final String DEFAULT_CONFIG_FILE_NAME = "bitnetx.properties";
    ...
    public Config(String defaultAppName, File defaultUserDataDir, String... args) {
        ...
        ArgumentAcceptingOptionSpec<String> bannedBtcNodesOpt =
                parser.accepts(BANNED_BTC_NODES, "List Bitnet nodes to ban")
                        .withRequiredArg()
                        .ofType(String.class)
        ...
        ArgumentAcceptingOptionSpec<Boolean> ignoreLocalBtcNodeOpt =
                parser.accepts(IGNORE_LOCAL_BTC_NODE,
                        "If set to true a Bitnet Core node running locally will be ignored")
                        .withRequiredArg()
                        .ofType(Boolean.class)
                        .defaultsTo(true);

        ArgumentAcceptingOptionSpec<String> bitcoinRegtestHostOpt =
                parser.accepts(BITCOIN_REGTEST_HOST, "Bitnet Core node when using BTC_REGTEST network")
                        .withRequiredArg()
                        .ofType(String.class)
                        .describedAs("host[:port]")
                        .defaultsTo("");
...
//			.defaultsTo("5l3amwlyfvf6bcfxpjihmkeaolr7qqduk6h7wlkphoixzohaaloxg7yd.onion:8000");
        ...
        ArgumentAcceptingOptionSpec<String> socks5ProxyBtcAddressOpt =
                parser.accepts(SOCKS_5_PROXY_BTC_ADDRESS, "A proxy address to be used for Bitnet network.")
                        .withRequiredArg()
                        .describedAs("host:port")
                        .defaultsTo("");
        ...
        ArgumentAcceptingOptionSpec<String> btcNodesOpt =
                parser.accepts(BTC_NODES, "Custom nodes used for BitnetJ as comma separated IP addresses.")
                        .withRequiredArg() 
                       .describedAs("ip[,...]")  
//                      .defaultsTo("129.146.9.111,34.64.109.126,34.125.77.92,54.193.21.65,radiopool.me,172.105.240.205");
        ...
        ArgumentAcceptingOptionSpec<String> socks5DiscoverModeOpt =
                parser.accepts(SOCKS5_DISCOVER_MODE, "Specify discovery mode for Bitnet nodes. " +
        ...
        ArgumentAcceptingOptionSpec<String> userAgentOpt =
                parser.accepts(USER_AGENT,
                        "User agent at bit node connections")
                        .withRequiredArg()
                        .defaultsTo("BitnetX");

        ArgumentAcceptingOptionSpec<Integer> numConnectionsForBtcOpt =
                parser.accepts(NUM_CONNECTIONS_FOR_BTC, "Number of connections to the Bitnet network")
                        .withRequiredArg()
                        .ofType(int.class)
                        .defaultsTo(DEFAULT_NUM_CONNECTIONS_FOR_BTC_PROVIDED);

        ArgumentAcceptingOptionSpec<String> rpcUserOpt =
                parser.accepts(RPC_USER, "Bitnetd rpc username")
                        .withRequiredArg()
                        .defaultsTo("rpcuser");

        ArgumentAcceptingOptionSpec<String> rpcPasswordOpt =
                parser.accepts(RPC_PASSWORD, "Bitnetd rpc password")
                        .withRequiredArg()
                        .defaultsTo("rpcpassword");

        ArgumentAcceptingOptionSpec<String> rpcHostOpt =
                parser.accepts(RPC_HOST, "Bitnetd rpc host")
                        .withRequiredArg()
                        .defaultsTo("127.0.0.1");

        ArgumentAcceptingOptionSpec<Integer> rpcPortOpt =
                parser.accepts(RPC_PORT, "Bitnetd rpc port")
                        .withRequiredArg()
                        .ofType(int.class)
                        .defaultsTo(9332);

        ArgumentAcceptingOptionSpec<Integer> rpcBlockNotificationPortOpt =
                parser.accepts(RPC_BLOCK_NOTIFICATION_PORT, "Bitnetd rpc port for block notifications")
                        .withRequiredArg()
                        .ofType(int.class)
                        .defaultsTo(4159);
        ...
        ArgumentAcceptingOptionSpec<String> rpcBlockNotificationHostOpt =
                parser.accepts(RPC_BLOCK_NOTIFICATION_HOST,
                        "Bitnetd rpc accepted incoming host for block notifications")
                        .withRequiredArg()
                        .defaultsTo("127.0.0.1");
        ...
        ArgumentAcceptingOptionSpec<Boolean> dumpBlockchainDataOpt =
                parser.accepts(DUMP_BLOCKCHAIN_DATA, "If set to true the blockchain data " +
                        "from RPC requests to Bitnet Core are stored as json file in the data dir.")
                        .withRequiredArg()
                        .ofType(boolean.class)
                        .defaultsTo(false);
        ...
        ArgumentAcceptingOptionSpec<Boolean> fullDaoNodeOpt =
                parser.accepts(FULL_DAO_NODE, "If set to true the node requests the blockchain data via RPC requests " +
                        "from Bitnet Core and provide the validated BSQ txs to the network. It requires that the " +
        ...
        ArgumentAcceptingOptionSpec<String> genesisTxIdOpt =
                parser.accepts(GENESIS_TX_ID, "Genesis transaction ID when not using the hard coded one")
                        .withRequiredArg()
                        .defaultsTo("83a994b9b978bc13670b300ea0116f099ed8db143a347c173cd4f095a6365369");
        ...
        try {
            ...
            // file and because it follows suit with Bitnet Core's config file behavior.
    ...
    private static String randomAppName() {
        try {
            File file = Files.createTempFile("BitnetX", "Temp").toFile();
    ...
    private static File tempUserDataDir() {
        try {
            return Files.createTempDirectory("BitnetXTempUserData").toFile();
        } catch (IOException ex) {
            throw new UncheckedIOException(ex);
        }
    }
